...
local EXTERNAL_AWARD_EVENT = "ILM_EXTERNAL_EVENT_ITEM_AWARDED"
...
local function StoreWoWDKPBotData(self)
    ...
    db.profiles = {}
    -- Fill Profiles
    for GUID,profile in pairs(ILM.MODULES.ProfileManager:GetProfiles()) do
       db.profiles[GUID] = {
            name = profile:Name(),
            class = profile:Class(),
            main = profile:Main()
...
local function InitializeGargulIntegration(self)
    local options = {
        ...
        global_gargul_integration = {
            name = ILM.L["Gargul Integration"],
            desc = ILM.L["Enable Gargul integration. This will allow Gargul to take control over some aspects of ILM (starting auction from Gargul, and awarding)."],
            type = "toggle",
    ...
    for _,selection in ipairs(selections) do
        options[selection.var] = {
            name = selection.name,
            desc = ILM.L["Action to take upon Gargul loot award event happening during raid."],
            type = "select",
...
local function CreateRCLCIntegration(self)
    ...
    local options = {
        ...
        global_rclc_integration = {
            name = ILM.L["RCLC Integration"],
            desc = ILM.L["Enable RCLC integration. This will allow awarding DKP/GP points on RCLC item award. Requires /reload."],
            type = "toggle",
            set = function(i, v)
        ...
        global_rclc_add_reason = {
            name = ILM.L["Create"],
            desc = ILM.L["Creates new trigger to be used during RCLC award."],
            type = "execute",
            func = (function(i)
                db[#db+1] = {
                    trigger = "",
        ...
        global_rclc_desc = {
            name = ILM.L["Action to take upon RCLC loot award event happening during raid. Trigger is based on RCLC award reason or player response if reason is not used. On Integration Enable the buttons will be prefilled with existing ones."],
            type = "description",
            width = "full",
            fontSize = "medium",
            order = 23
    ...
    for conditionId, condition in ipairs(db) do
        options[prefix.."trigger"..conditionId] = {
            name = ILM.L["if reason/response contains"],
            type = "input",
...
function Integration:InitializeConfigs()
    local options = {
        global_wodkpbot_integration = {
            name = ILM.L["WoW DKP Bot Integration"],
            desc = ILM.L["Enable WoW DKP Bot Integration. This will result in additional data stored upon logout."],
            type = "toggle",
    ...
    ILM.MODULES.ConfigManager:Register(PRIV.CONSTANTS.INTEGRATIONS, options)
end
...
local awardToTierMap = {
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_BASE]   = ILM.CONSTANTS.SLOT_VALUE_TIER.BASE,
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_SMALL]  = ILM.CONSTANTS.SLOT_VALUE_TIER.SMALL,
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_MEDIUM] = ILM.CONSTANTS.SLOT_VALUE_TIER.MEDIUM,
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_LARGE]  = ILM.CONSTANTS.SLOT_VALUE_TIER.LARGE,
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_MAX]    = ILM.CONSTANTS.SLOT_VALUE_TIER.MAX
}
...
local function ExternalAwardEventHandler(_, data)
    ...
    local raid = ILM.MODULES.RaidManager:GetProfileRaid(profile:GUID())
    if not raid then
        LOG:Debug("Gargul item awarded outside of raid")
        return
    end
    if action == PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.DISENCHANT then
        ILM.MODULES.LootManager:DisenchantItem(raid, itemLink, true)
        return
    end
    ...
    ILM.MODULES.LootManager:AwardItem(raid, player, itemLink, itemId, extra, value)
end
...
local function RCLCAwardMessageHandler(eventName, _, winner, _, link, response)
    ...
    local profile = ILM.MODULES.ProfileManager:GetProfileByName(winner)
    if not profile then
    ...
    if action == PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.DISENCHANT then
        ILM.MODULES.LootManager:DisenchantItem(raid, link, true)
        return
    end
    ...
    ILM.MODULES.LootManager:AwardItem(raid, winner, link, itemId, extra, value)
end

local function RCLC_PR(rowFrame, frame, data, cols, row, realrow, column, fShow, table, ...)
    local value = 0
    local profile = ILM.MODULES.ProfileManager:GetProfileByName(UTILS.Disambiguate(data[realrow].name or ""))
    if profile then
...
function Integration:Initialize()
    ...
    ILM.MODULES.EventManager:RegisterWoWEvent({"PLAYER_LOGOUT"}, (function()
        StoreWoWDKPBotData(self)
    end))
    -- External award integration
    ILM.MODULES.EventManager:RegisterEvent(EXTERNAL_AWARD_EVENT, ExternalAwardEventHandler)
    -- RCLC award integartion
    ILM.MODULES.EventManager:RegisterMessage(RCLC_AWARD_EVENT, RCLCAwardMessageHandler)
    if self:GetRCLCIntegration() and isRCLC and RCLootCouncil then
        ...
        if RCVF then
            ...
            local column = {name = "PR", colName = "ilmpr", width = 50, align = "CENTER", DoCellUpdate = RCLC_PR, sortnext = sortnext}
            table.insert(RCVF.scrollCols, column)
            local prColumn = RCVF:GetColumnIndexFromName("ilmpr")
...
PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTIONS_GUI = {
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.NONE]                = ILM.L["None"],
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_FREE]      = ILM.L["Award for Free"],
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_BASE]      = ILM.L["Award for Base"],
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_SMALL]     = ILM.L["Award for Small"],
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_MEDIUM]    = ILM.L["Award for Medium"],
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_LARGE]     = ILM.L["Award for Large"],
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.AWARD_FOR_MAX]       = ILM.L["Award for Max"],
    [PRIV.CONSTANTS.EXTERNAL_LOOT_AWARD_ACTION.DISENCHANT]          = ILM.L["Disenchant"]
}
