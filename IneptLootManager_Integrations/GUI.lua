...
local EXPORT_DATA_TYPE_GUI = {
    -- [PRIV.CONSTANTS.EXPORT_DATA_TYPE.CONFIGS] = ILM.L["Configuration"],
    [PRIV.CONSTANTS.EXPORT_DATA_TYPE.STANDINGS] = ILM.L["Standings"],
    [PRIV.CONSTANTS.EXPORT_DATA_TYPE.POINT_HISTORY] = ILM.L["Point History"],
    [PRIV.CONSTANTS.EXPORT_DATA_TYPE.LOOT_HISTORY] = ILM.L["Loot History"],
    -- [PRIV.CONSTANTS.EXPORT_DATA_TYPE.RAIDS] = ILM.L["Raids"],

}

local FORMAT_VALUES_GUI =  {
    [PRIV.CONSTANTS.FORMAT_VALUE.XML] = "XML",
    -- [PRIV.CONSTANTS.FORMAT_VALUE.CSV] = "CSV",
    [PRIV.CONSTANTS.FORMAT_VALUE.TMB] = "TMB",
    [PRIV.CONSTANTS.FORMAT_VALUE.JSON] = "JSON"
}
...
local function InitializeDB(self)
    self.db = ILM.MODULES.Database:GUI('export', {
        location = {nil, nil, "CENTER", 0, 0 },
        export_config = {
            data = {},
            format = PRIV.CONSTANTS.FORMAT_VALUE.XML,
            timerange = {
                begin = {
                    day = 1,
                    month = 1,
...
local function GetProfileList()
    if redoProfileList then
        profileList = {}
        for _, profile in pairs(ILM.MODULES.ProfileManager:GetProfiles()) do
...
local function Create(self)
    ...
    options.args.export_config_group = {
        ...
        args = {
            export_data_type = {
                ...
                disabled = (function() return (self.db.export_config.format == PRIV.CONSTANTS.FORMAT_VALUE.TMB) end),
            ...
            format = {
                ...
                set = function(i, v)
                    ...
                    if self.db.export_config.format == PRIV.CONSTANTS.FORMAT_VALUE.TMB then
                        self.db.export_config.data[PRIV.CONSTANTS.EXPORT_DATA_TYPE.STANDINGS] = false
                        self.db.export_config.data[PRIV.CONSTANTS.EXPORT_DATA_TYPE.POINT_HISTORY] = false
                        self.db.export_config.data[PRIV.CONSTANTS.EXPORT_DATA_TYPE.LOOT_HISTORY] = true
                    end
                end,
            ...
            timerange_set_last_week = {
                name =  ILM.L["Last week"],
                desc = string.format(ILM.L["Begin %d days ago, finish today."], 7),
                type = "execute",
                func = (function() SetOffsetTime(self, 604800) end),
                order = 11,
            },
            timerange_set_last_month = {
                name = ILM.L["Last month"],
                desc = string.format(ILM.L["Begin %d days ago, finish today."], 31),
                type = "execute",
                func = (function() SetOffsetTime(self, 2678400) end),
                order = 12,
            },
            timerange_set_last_year = {
                name =  ILM.L["Last year"],
                desc = string.format(ILM.L["Begin %d days ago, finish today."], 365),
                type = "execute",
    ...
    options.args.roster_select_group = {
        ...
        args = {
            export_rosters = {
                name = ILM.L["Select Rosters to export"],
                type = "multiselect",
                set = function(i, k, v)
                    k = tonumber(k) or 0
                    if k ~= 0 then
    ...
    options.args.profiles_select_group = {
        ...
        args = {
            export_rosters = {
                name = ILM.L["Select Profiles to export"],
                type = "multiselect",
                set = function(i, k, v)
                    k = tonumber(k) or 0
                    if k ~= 0 then
            ...
            execute_export = {
                ...
                func = (function()
                    ...
                    if self.roster_select_list[ALL] then
                        for _, roster in pairs(ILM.MODULES.RosterManager:GetRosters()) do
                    ...
                    if self.profile_select_list[ALL] then
                        for GUID, _ in pairs(ILM.MODULES.ProfileManager:GetProfiles()) do
                            tinsert(rosters, GUID)
                        end
                    else
                        for selectedId, status in pairs(self.profile_select_list) do
                            if status then
                                local profile = ILM.MODULES.ProfileManager:GetProfileByName(UTILS.RemoveColorCode(profileList[selectedId]))
                    ...
                    local config = PRIV.MODELS.ExportConfiguration:New(
                    ...
                    PRIV.MODULES.Integration:Export(
                        config, -- Config
    ...
    AceConfigRegistry:RegisterOptionsTable(ILM.L["Export"], options)
    AceConfigDialog:Open(ILM.L["Export"], parent)
...
function ExportGUI:RegisterSlash()
    local options = {
        export = {
            type = "execute",
            name = "Export",
            desc = ILM.L["Toggle export window display"],
            handler = self,
            func = "Toggle",
        }
    }
