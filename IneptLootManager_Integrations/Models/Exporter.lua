...
local function ILM_TMB_encode(data)
...
local ENCODERS =  {
    ...
    [PRIV.CONSTANTS.FORMAT_VALUE.TMB]  = (function(output) return ILM_TMB_encode(output) end),
    ...
    [PRIV.CONSTANTS.EXPORT_DATA_TYPE.STANDINGS] = (function(self)
        ...
        if filterPlayers then
            for _, roster in ipairs(self.dataInfo.rosters) do
                local roster_data = {
                    uid = roster:UID(),
                    name = ILM.MODULES.RosterManager:GetRosterNameByUid(roster:UID()),
            ...
            for _, roster in ipairs(self.dataInfo.rosters) do
                ...
                for GUID,_ in pairs(self.dataInfo.profiles) do
                    ...
                    for _, loot in ipairs(lootList) do
                        local profile = ILM.MODULES.ProfileManager:GetProfileByGUID(loot:OwnerGUID())
                        if profile and self:TimestampInRange(loot:Timestamp()) then
                            local awardedBy = ILM.MODULES.ProfileManager:GetProfileByGUID(getGuidFromInteger(loot:Creator()))
                ...
                for GUID,_ in pairs(self.dataInfo.profiles) do
                    ...
                    for _, history in ipairs(historyList) do
                        ...
                        if profile and self:TimestampInRange(history:Timestamp()) then
                            ...
                            local awardedBy = ILM.MODULES.ProfileManager:GetProfileByGUID(getGuidFromInteger(history:Creator()))
                            tinsert(roster_data.pointHistory.point, {
                                points = history:Value(),
                                player = profile:Name(),
                                reason = ILM.CONSTANTS.POINT_CHANGE_REASONS.ALL[history:Reason()] or "",
                                note = note,
                                awardedBy = awardedBy and awardedBy:Name() or "",
...
function Exporter:Run(completeCallback, updateCallback)
    ...
    for _, UID in ipairs(self.config.rosters) do
        local roster = ILM.MODULES.RosterManager:GetRosterByUid(UID)
        if roster then
    ...
    LOG:Message("Exporting data between: %s and %s",
        date(ILM.L["%Y/%m/%d %a %H:%M:%S"], self.config.timerange.begin),
        date(ILM.L["%Y/%m/%d %a %H:%M:%S"], self.config.timerange.finish)
    )
