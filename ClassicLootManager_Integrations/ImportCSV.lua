        ...
        if line and strlen(line) > 0 then
            ...
            if UTILS.GetItemInfoInstant(itemId) then
                data[#data+1] = {
                    itemId = itemId,
                    values = {
                        [CLM.CONSTANTS.SLOT_VALUE_TIER.BASE]    = tonumber(b) or 0,
                        [CLM.CONSTANTS.SLOT_VALUE_TIER.SMALL]   = tonumber(s) or 0,
                        [CLM.CONSTANTS.SLOT_VALUE_TIER.MEDIUM]  = tonumber(m) or 0,
                        [CLM.CONSTANTS.SLOT_VALUE_TIER.LARGE]   = tonumber(l) or 0,
                        [CLM.CONSTANTS.SLOT_VALUE_TIER.MAX]     = tonumber(x) or 0
                    }
                }
            else
    ...
    if self.importExecuted then
        if self.data then
            ...
            for _,data in ipairs(self.data) do
                ...
                order = order + 1
                for _, tier in ipairs(CLM.CONSTANTS.SLOT_VALUE_TIERS_ORDERED) do
                    options.args[prefix .. "tier" .. tostring(tier)] = {
                        ...
                        name = (CLM.CONSTANTS.SLOT_VALUE_TIERS_GUI[tier] or ""),
                    }
                    order = order + 1
                end
            end
...
PRIV.MODULES.ItemValueOverrideImporter = ItemValueOverrideImporter