...
local function CreateStandingsDisplay2(self)
    -- Profile Scrolling Table
    local columns = {
        {   name = CLM.L["Name"],   width = 100 },
        {   name = CLM.L["DKP"],    width = 80, sort = ScrollingTable.SORT_DSC, color = {r = 0.0, g = 0.93, b = 0.0, a = 1.0} },
    ...
    StandingsGroup:AddChild(ST)
    self.st = ST
    self.st:SetDisplayCols(columns)
    self.st:EnableSelection(true)
    -- self.st.frame:SetPoint("TOPLEFT", RosterSelectorDropDown.frame, "TOPLEFT", 0, -60)
    self.st:SetBackdropColor(0.1, 0.1, 0.1, 0.1)
    -- Dropdown menu
    local RightClickMenu = CLM.UTILS.GenerateDropDownMenu({
        {
            title = CLM.L["Add to standby"],
    ...
    -- OnEnter handler -> on hover
    local OnEnterHandler = (function (rowFrame, cellFrame, data, cols, row, realrow, column, table, ...)
        local status = self.st:GetScrollingTable().DefaultEvents["OnEnter"](rowFrame, cellFrame, data, cols, row, realrow, column, table, ...)
        local rowData = self.st:GetRow(realrow)
        if not rowData or not rowData.cols then return status end
        local tooltip = self.tooltip
        if not tooltip then return end
    ...
    -- OnLeave handler -> on hover out
    local OnLeaveHandler = (function (rowFrame, cellFrame, data, cols, row, realrow, column, table, ...)
        local status = self.st:GetScrollingTable().DefaultEvents["OnLeave"](rowFrame, cellFrame, data, cols, row, realrow, column, table, ...)
        self.tooltip:Hide()
        return status
    end)
    -- OnClick handler -> click
    local OnClickHandler = function(rowFrame, cellFrame, data, cols, row, realrow, column, table, button, ...)
        ...
        if not isSelected then
            status = self.st:GetScrollingTable().DefaultEvents["OnClick"](rowFrame, cellFrame, data, cols, row, realrow, column, table, rightButton and "LeftButton" or button, ...)
        end
        if rightButton then
            UTILS.LibDD:CloseDropDownMenus()
            UTILS.LibDD:ToggleDropDownMenu(1, nil, RightClickMenu, cellFrame, -20, 0)
...
function UnifiedGUI:Create()
    ...
    f:AddChild(CreateStandingsDisplay2(self))
...
function UnifiedGUI:Refresh(visible)
    ...
    -- AceConfigDialog:Open(REGISTRY, self.ManagementOptions)
    self.numInRoster = #data
    UpdateStatusText(self)
end

function UnifiedGUI:GetCurrentRoster()
...
function UnifiedGUI:RefreshRosters()
    LOG:Trace("UnifiedGUI:RefreshRosters()")
