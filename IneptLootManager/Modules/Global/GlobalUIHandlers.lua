...
local function Handler(_, _, rollID, rollTime, lootHandle)
    ...
    if ILM.GlobalConfigs:GetRollType() == CONSTANTS.LOOT_ROLL_TYPE_IGNORE then
        return
    end
    ...
    if isLegendary and not ILM.GlobalConfigs:GetLootRollIncludeLegendary() then
        return
    end

    if isStacking and not ILM.GlobalConfigs:GetLootRollIncludeStacking() then
        return
    end

    if isPattern and not ILM.GlobalConfigs:GetLootRollIncludePatterns() then
        return
    end

    if isMount and not ILM.GlobalConfigs:GetLootRollIncludeMounts() then
        return
    end

    if isToy and not ILM.GlobalConfigs:GetLootRollIncludeToys() then
        return
    end

    if isCosmetic and not ILM.GlobalConfigs:GetLootRollIncludeCosmetics() then
        return
    end

    -- Handle ordered Any case
    if ILM.GlobalConfigs:GetRollType() == CONSTANTS.LOOT_ROLL_TYPE_ANY then
        ...
        RollOnLoot(rollID, ILM.GlobalConfigs:GetRollType())
    end
...
function GlobalUIHandlers:Initialize()
    ILM.MODULES.EventManager:RegisterWoWEvent({"START_LOOT_ROLL"}, Handler)
    ILM.MODULES.EventManager:RegisterWoWEvent({"CONFIRM_LOOT_ROLL"}, AutoConfirm)
end
ILM.GlobalUIHandlers = GlobalUIHandlers
