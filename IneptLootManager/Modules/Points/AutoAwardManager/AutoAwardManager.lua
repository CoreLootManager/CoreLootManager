...
local multiWoWDifficultyIDs = {
    [0]   = ILM.DifficultyID.OPEN_WORLD,    -- World Bosses
    [3]   = ILM.DifficultyID.P10,           -- 10 man
    [175] = ILM.DifficultyID.P10,           -- 10 man
    [197] = ILM.DifficultyID.P10,           -- 10 man SoD
    [198] = ILM.DifficultyID.P10,           -- 10 man SoD
    [5]   = ILM.DifficultyID.P10HC,         -- 10 man HC
    [148] = ILM.DifficultyID.P20,           -- 20 man
    [185] = ILM.DifficultyID.P20,           -- 20 man
    [215] = ILM.DifficultyID.P20,           -- 20 man
    [226] = ILM.DifficultyID.P20,           -- 20 man SoD
    [4]   = ILM.DifficultyID.P25,           -- 25 man
    [176] = ILM.DifficultyID.P25,           -- 25 man
    [6]   = ILM.DifficultyID.P25HC,         -- 25 man HC
    [9]   = ILM.DifficultyID.P40,           -- 40 man
    [186] = ILM.DifficultyID.P40,           -- 40 man
    [14]  = ILM.DifficultyID.NORMAL,        -- Retail Normal
    [15]  = ILM.DifficultyID.HEROIC,        -- Retail Heroic
    [16]  = ILM.DifficultyID.MYTHIC,        -- Retail Mythic
    [17]  = ILM.DifficultyID.LFR,           -- Retail LFR,
    [231] = ILM.DifficultyID.NORMAL,        -- SoD Flex
}
...
local function awardBonusValue(id, value)
    if value ~= 0 then
        ILM.MODULES.PointManager:UpdateRaidPoints(ILM.MODULES.RaidManager:GetRaid(), value, CONSTANTS.POINT_CHANGE_REASON.BOSS_KILL_BONUS, CONSTANTS.POINT_MANAGER_ACTION.MODIFY, tostring(id), CONSTANTS.POINT_CHANGE_TYPE.POINTS)
    end
end

local POPUP = "ILM_BOSS_KILL_BONUS_HARD_MODE"

StaticPopupDialogs[POPUP] = {
    text = ILM.L["Select |cffeeee00%s|r encounter difficulty for bonus value award (%s)."],
    button1 = ILM.L["Normal Mode"],
    button2 = ILM.L["Hard Mode"],
...
local function awardBossKillBonus(id, difficultyId)
    ...
    if ILM.MODULES.RaidManager:IsInActiveRaid() then
        ...
        local config = ILM.MODULES.RaidManager:GetRaid():Configuration()
        if config:Get("bossKillBonus") then
            ...
            if ILM.EncounterHasHardMode[id] and hardModeBonus ~= normalBonus then
                local bonusValueString  = string.format("|cff44ee44%d|r/|cffee4444%d|r", normalBonus, hardModeBonus)
                local dialog = StaticPopup_Show(POPUP, ILM.EncounterIDsMap[id], bonusValueString)
                if dialog then
...
local function handleIntervalBonus(self)
    ...
    if not ILM.MODULES.RaidManager:IsInProgressingRaid() then return end
    -- Validate roster
    ...
    if award then
        ILM.MODULES.PointManager:UpdateRaidPoints(raid, value, CONSTANTS.POINT_CHANGE_REASON.INTERVAL_BONUS, CONSTANTS.POINT_MANAGER_ACTION.MODIFY, nil, CONSTANTS.POINT_CHANGE_TYPE.POINTS)
    end
end

local instancesToWorkaround = ILM.UTILS.Set({548})
local encountersToWorkaround = ILM.UTILS.Set({HYDROSS_ENCOUNTER_ID})

local isWorkaroundRegistered = false
local function registerWorkaroundHandler(self)
    if isWorkaroundRegistered then return end
    ILM.MODULES.EventManager:RegisterWoWEvent({"COMBAT_LOG_EVENT_UNFILTERED"}, (function(...)
...
function AutoAwardManager:Initialize()
    ...
    ILM.MODULES.EventManager:RegisterWoWEvent({"ENCOUNTER_START"}, (function(_, _, _, id, ...)
        if encountersToWorkaround[id] then
            registerWorkaroundHandler(self)
        end
    end))
    -- Handle boss kill when not in encounter
    ILM.MODULES.EventManager:RegisterWoWEvent({"BOSS_KILL"}, (function(...)
        handleBossKill(self, ...)
    end))
    -- Boss workarounds
...
ILM.MODULES.AutoAwardManager = AutoAwardManager

--@do-not-package@
ILM.awardBossKillBonus = awardBossKillBonus
--@end-do-not-package@
