...
local getGuidFromInteger = ILM.UTILS.getGuidFromInteger
local ValidateIntegerGUID = ILM.UTILS.ValidateIntegerGUID
    ...
    if self.profiles == nil then
        ...
        for _,target in ipairs(targets) do
            ...
            if ValidateIntegerGUID(target) then
                profile = ILM.MODULES.ProfileManager:GetProfileByGUID(getGuidFromInteger(target))
            else
                profile = ILM.MODULES.ProfileManager:GetProfileByGUID(target)
            end
            if profile then
...
function FakePointHistory:New(targets, timestamp, value, reason, creator, note, type)
    ...
    o.note = note or ""
    o.type = type or ILM.CONSTANTS.POINT_CHANGE_TYPE.POINTS
...
function FakePointHistory:Profiles()
    ...
    if self.profiles == nil then
        ...
        for _,target in ipairs(self.targets) do
            ...
            -- Maybe the caching should be done in GUI module?
            -- TODO: resolve this
            -- local profile = ILM.MODULES.ProfileManager:GetProfileByGUID(getGuidFromInteger(target))
            -- if not profile then
            --     profile = ILM.MODULES.ProfileManager:GetProfileByGUID(target)
            -- end
            local profile
...
ILM.MODELS.PointHistory = PointHistory
ILM.MODELS.FakePointHistory = FakePointHistory