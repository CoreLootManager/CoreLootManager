...
local REGISTRY = "ilm_reg_altmainlinkinggui_opt"
...
local function InitializeDB(self)
    self.db = ILM.MODULES.Database:GUI('altmainlinking', {
...
local function UpdateOptions(self)
    ...
    options.args = {
        ...
        link = {
            ...
            type = "execute",
            func = (function()
                ILM.MODULES.ProfileManager:MarkAsAltByNames(self.alt, self.main)
                self:Refresh()
            end),
            confirm = (function()
                return string.format(
                    ILM.L["Are you sure, you want to link %s as alt of %s?"],
        ...
        unlink = {
            name = ILM.L["Unlink Alt"],
            type = "execute",
            func = (function()
                ILM.MODULES.ProfileManager:MarkAsAltByNames(self.alt, "")
                self:Refresh()
            end),
            confirm = (function()
                return string.format(
                    ILM.L["Are you sure, you want to unlink %s from its main?"],
                    UTILS.ColorCodeText(Ambiguate(self.alt, "none"), "FFD100")
                )
            end),
            order = 4,
...
local function Create(self)
    ...
    f:SetTitle(ILM.L["Link Alt to Main"])
    f:SetStatusText("")
    f:SetLayout("Flow")
    ...
    -- Hide by default
    UTILS.MakeFrameCloseOnEsc(f.frame, "ILM_AltMainLink_GUI")
    f:Hide()
end
...
ILM.GUI.AltMainLinking = AltMainLinkingGUI