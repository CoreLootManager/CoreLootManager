...
function RaidManager:Initialize()
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.RAID.Create,
        (function(entry)
            LOG:TraceAndCount("mutator(RaidCreate)")
            local raidUid = entry:uuid()
            local name = entry:name()
            local config = ILM.MODELS.RosterConfiguration:New()
            config:inflate(entry:config())
            local rosterUid = entry:rosterUid()

            local roster = ILM.MODULES.RosterManager:GetRosterByUid(rosterUid)
            if not roster then
            ...
            local raid = ILM.MODELS.Raid:New(raidUid, name, roster, config, creator, entry)
...
function RaidManager:UpdateProfileCurentRaid(GUID, raid)
    ...
    if ILM.MODULES.ProfileManager:GetProfileByGUID(GUID) then
...
function RaidManager:CreateRaid(roster, name, config)
    ...
    if not UTILS.typeof(config, ILM.MODELS.RosterConfiguration) then
        LOG:Error("RaidManager:CreateRaid(): Missing valid configuration")
        return
    end
    if not ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.ASSISTANT) then
        LOG:Message(ILM.L["You are not allowed to create raids."])
        return
    end
    if self:IsInActiveRaid() then
        LOG:Message(ILM.L["You are already in an active raid. Leave or finish it before creating new one."])
        return
    end
    --[===[@non-debug@
    if ILM.MODULES.LedgerManager:IsTimeTraveling() then
        LOG:Message(ILM.L["Raid management is disabled during time traveling."])
        return
    end
    --@end-non-debug@]===]
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.RAID.Create:new(roster:UID(), name, config))
end

function RaidManager:StartRaid(raid)
    LOG:Trace("RaidManager:StartRaid()")
    if not UTILS.typeof(raid, ILM.MODELS.Raid) then
        LOG:Message(ILM.L["Missing valid raid"])
        return
    end
    if not ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.ASSISTANT) then
        LOG:Message(ILM.L["You are not allowed to start raid."])
        return
    end
    if raid:Status() ~= CONSTANTS.RAID_STATUS.CREATED then
        LOG:Message(ILM.L["You can only start a freshly created raid."])
        return
    end
    if ILM.MODULES.LedgerManager:IsTimeTraveling() then
        LOG:Message(ILM.L["Raid management is disabled during time traveling."])
    ...
    if (self:GetRaid() ~= raid) or not IsInRaid() then
        LOG:Message(ILM.L["You are not in the raid."])
        return
    end
    --@end-non-debug@]===]

    -- Lazy fill raid roster
    ILM.MODULES.RosterManager:AddFromRaidToRoster(raid:Roster())
    ...
    for i=1,MAX_RAID_MEMBERS do
        ...
        if name then
            local profile = ILM.MODULES.ProfileManager:GetProfileByName(UTILS.Disambiguate(name))
            if profile then
    ...
    -- Fill Standby
    local standby = {}
    for GUID,_ in pairs(ILM.MODULES.StandbyStagingManager:GetStandby(raid:UID())) do
    ...
    if #standby > 0 then
        ILM.MODULES.RosterManager:AddProfilesToRoster(raid:Roster(), standby)
    end
    -- Start Raid
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.RAID.Start:new(raid:UID(), players, standby), true)
    if ILM.GlobalConfigs:GetRaidWarning() and IsInRaid() then
        UTILS.SendChatMessage(string.format(ILM.L["Raid [%s] started"], raid:Name()) , CHAT_MESSAGE_CHANNEL)
    end
    self:HandleRosterUpdateEvent()
    -- On Time Bonus
    ...
    if roster then
        ...
        if config:Get("onTimeBonus") and onTimeBonusValue > 0 then
            ILM.MODULES.PointManager:UpdateRaidPoints(raid, onTimeBonusValue, CONSTANTS.POINT_CHANGE_REASON.ON_TIME_BONUS, CONSTANTS.POINT_MANAGER_ACTION.MODIFY, nil, CONSTANTS.POINT_CHANGE_TYPE.POINTS)
        end
    end
end
...
function RaidManager:EndRaid(raid)
    ...
    if not raid:Status() then
        LOG:Message(ILM.L["You can only end an active raid."])
        return
    end
    if ILM.MODULES.LedgerManager:IsTimeTraveling() then
        LOG:Message(ILM.L["Raid management is disabled during time traveling."])
    ...
    if roster then
        ...
        if config:Get("raidCompletionBonus") and raidCompletionBonusValue > 0 then
            ILM.MODULES.PointManager:UpdateRaidPoints(raid, raidCompletionBonusValue, CONSTANTS.POINT_CHANGE_REASON.RAID_COMPLETION_BONUS, CONSTANTS.POINT_MANAGER_ACTION.MODIFY, nil, CONSTANTS.POINT_CHANGE_TYPE.POINTS)
        end
    end
    -- End raid
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.RAID.End:new(raid:UID()), true)

    if ILM.GlobalConfigs:GetRaidWarning() and IsInRaid() then
        UTILS.SendChatMessage(string.format(ILM.L["Raid [%s] ended"], raid:Name()) , CHAT_MESSAGE_CHANNEL)
    end
end
...
function RaidManager:JoinRaid(raid)
    ...
    if not ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.ASSISTANT) then
        LOG:Message(ILM.L["You are not allowed to join raid."])
        return
    end
    if not raid:IsActive() then
        LOG:Message(ILM.L["You can only join an active raid."])
        return
    end
    if raid == self:GetRaid() then
        LOG:Message(ILM.L["You can only join different raid than your current one."])
        return
    end
    if ILM.MODULES.LedgerManager:IsTimeTraveling() then
        LOG:Message(ILM.L["Raid management is disabled during time traveling."])
    ...
    local myProfile = ILM.MODULES.ProfileManager:GetMyProfile()
    if myProfile == nil then
        error("My profile is nil")
    end
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.RAID.Update:new(raid:UID(), {}, {ILM.MODULES.ProfileManager:GetMyProfile()}), true)
end
...
function RaidManager:AddToStandby(raid, standby)
    ...
    if not ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.ASSISTANT) then
        LOG:Message(ILM.L["You are not allowed to control raid."])
        return
    end
    if raid:Status() ~= CONSTANTS.RAID_STATUS.IN_PROGRESS then
        LOG:Message(ILM.L["You can only add players to standby of a progressing raid."])
        return
    end
    if ILM.MODULES.LedgerManager:IsTimeTraveling() then
        LOG:Message(ILM.L["Raid management is disabled during time traveling."])
    ...
    local entry = ILM.MODELS.LEDGER.RAID.Update:new(raid:UID(), {}, {}, standby_filtered, {})
...
function RaidManager:RemoveFromStandby(raid, removed)
    ...
    local entry = ILM.MODELS.LEDGER.RAID.Update:new(raid:UID(), {}, {}, {}, removed_filtered)
    ILM.MODULES.LedgerManager:Submit(entry, true)
end
...
function RaidManager:GetUniquePlayersListInRaid(raid)
    ...
    for GUID,_ in pairs(raid.players) do
        ...
        if profile then
            ...
            local main = ILM.MODULES.ProfileManager:GetProfileByGUID(profile:Main())
...
function RaidManager:GetDisenchanter()
    ...
    local disenchanters = ILM.GlobalConfigs:GetDisenchanterList()
    for _, disenchanter in ipairs(disenchanters) do
        local deProfile = ILM.MODULES.ProfileManager:GetProfileByName(disenchanter)
...
function RaidManager:RegisterEventHandling()
    ...
    ILM.MODULES.EventManager:RegisterWoWBucketEvent({"RAID_ROSTER_UPDATE", "GROUP_ROSTER_UPDATE", "READY_CHECK"}, 3, (function(...)
        self:HandleRosterUpdateEvent()
    end))
    ILM.MODULES.EventManager:RegisterWoWBucketEvent({"PARTY_LOOT_METHOD_CHANGED","PLAYER_ROLES_ASSIGNED"}, 3, (function(...)
...
function RaidManager:HandleRosterUpdateEvent()
    ...
    if ILM.MODULES.LedgerManager:IsTimeTraveling() then return end
    reentrance = true
...
function RaidManager:EnableAutoAwarding()
    ...
    if bossKillBonus or intervalBonus then
        ILM.MODULES.AutoAwardManager:Enable()
    end

    if bossKillBonus then
        ILM.MODULES.AutoAwardManager:EnableBossKillBonusAwarding()
    end

    if intervalBonus then
        ILM.MODULES.AutoAwardManager:EnableIntervalBonusAwarding()
    end
end

function RaidManager:DisableAutoAwarding()
    LOG:Trace("RaidManager:DisableAutoAwarding()")
    ILM.MODULES.AutoAwardManager:DisableBossKillBonusAwarding()
    ILM.MODULES.AutoAwardManager:DisableIntervalBonusAwarding()
    ILM.MODULES.AutoAwardManager:Disable()
end

function RaidManager:UpdateGameRaidInformation()
    local lootmethod, _, masterlooterRaidID = UTILS.GetLootMethod()
    LOG:Info("Loot method: %s (id: %s)", lootmethod, masterlooterRaidID)
    self.RaidAssistants = {}
    self.IsMasterLootSystem = false
    self.IsGroupLootSystem = false
    if UTILS.IsLootMasterLootMethod() and masterlooterRaidID then
    ...
    elseif UTILS.IsGroupLootMethod() then
        self.IsGroupLootSystem = true
    end
...
function RaidManager:UpdateRaiderList()
    ...
    ILM.MODULES.RosterManager:AddFromRaidToRoster(self:GetRaid():Roster())
    ...
    if (#joiners > 0) or (#leavers > 0) then
        ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.RAID.Update:new(raid:UID(), joiners, leavers), true) -- force update
    end
end
...
function RaidManager:IsRaidOwner(name)
    ...
    if not ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.ASSISTANT, name) then
        isOwner = false
...
CONSTANTS.RAID_STATUS_GUI = {
    [CONSTANTS.RAID_STATUS.CREATED]     = ILM.L["Created"],
    [CONSTANTS.RAID_STATUS.IN_PROGRESS] = ILM.L["In Progress"],
    [CONSTANTS.RAID_STATUS.FINISHED]    = ILM.L["Finished"],
    [CONSTANTS.RAID_STATUS.STALE]       = ILM.L["Stale"]
}
...
ILM.MODULES.RaidManager = RaidManager
