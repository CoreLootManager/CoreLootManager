...
local function HandleSubscribe(self, data, sender)
    ...
    local raid = ILM.MODULES.RaidManager:GetRaidByUid(raidUid)
    if not raid then
...
local function HandleRevoke(self, data, sender)
    ...
    if profile then
        ...
        if updated then
            LOG:Message(ILM.L["%s has %s standby"],
                        UTILS.ColorCodeText(profile:Name(), UTILS.GetClassColor(profile:Class()).hex),
                        UTILS.ColorCodeText(ILM.L["revoked"], "cc4444"))
        end
    else
        LOG:Warning("Missing profile for player %s", sender)
    end
...
function StandbyStagingManager:Initialize()
    ...
    ILM.MODULES.Comms:Register(ILM.COMM_CHANNEL.STANDBY, (function(rawMessage, distribution, sender)
        local message = ILM.MODELS.StandbyStagingCommStructure:New(rawMessage)
...
function StandbyStagingManager:SignupToStandby(raidUid)
    ...
    local message = ILM.MODELS.StandbyStagingCommStructure:New(
        CONSTANTS.STANDBY_STAGING_COMM.TYPE.SUBSCRIBE,
        ILM.MODELS.StandbyStagingCommSubscribe:New(raidUid))
    ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.STANDBY, message, CONSTANTS.COMMS.DISTRIBUTION.GUILD)
    LOG:Message(ILM.L["Standby %s has been sent"],
                UTILS.ColorCodeText(ILM.L["request"], "44cc44"))
end
...
function StandbyStagingManager:RevokeStandby(raidUid)
    ...
    local message = ILM.MODELS.StandbyStagingCommStructure:New(
        ...
        ILM.MODELS.StandbyStagingCommRevoke:New(raidUid))
    ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.STANDBY, message, CONSTANTS.COMMS.DISTRIBUTION.GUILD)
    LOG:Message(ILM.L["Standby %s has been sent"],
    UTILS.ColorCodeText(ILM.L["revoke"], "cc4444"))
end
...
ILM.MODULES.StandbyStagingManager = StandbyStagingManager