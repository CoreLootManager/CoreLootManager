...
local function InitializeDB(self)
    self.db = ILM.MODULES.Database:GUI('tradeList', {
...
function TradeListGUI:Initialize()
    ...
    RightClickMenu = ILM.UTILS.GenerateDropDownMenu(
        {
            {
                title = ILM.L["Open trade"],
                func = (function()
                    local rowData = self.st:GetRow(self.st:GetSelection())
                    if not rowData or not rowData.cols then return end
                    ILM.MODULES.AutoAssign:InitiateTrade(ST_GetTradeTarget(rowData))
                end),
                color = "00cc00"
            },
            {
            ...
            },
            {
                ...
                func = (function()
                    local rowData = self.st:GetRow(self.st:GetSelection())
                    if not rowData or not rowData.cols then return end
                    ILM.MODULES.AutoAssign:Remove(ST_GetItemLink(rowData), ST_GetTradeTarget(rowData))
                end),
                color = "cc0000"
            },
            -- {
            ...
            -- },
            -- {
            --     title = ILM.L["Remove all"],
            --     func = (function()
            --         -- ILM.MODULES.AuctionHistoryManager:Wipe()
            --     end),
            --     color = "cc0000"
            -- }
        },
    ...
    local OnClickHandler = function(rowFrame, cellFrame, data, cols, row, realrow, column, table, button, ...)
        ...
        if rightButton then
            ...
            elseif IsModifiedClick() then
                local message = string.format(ILM.L["%s trade me for %s"], ST_GetTradeTarget(rowData), ST_GetItemLink(rowData))
...
function TradeListGUI:Refresh(visible)
    ...
    local data = {}
    local tracked = ILM.MODULES.AutoAssign:GetTracked()
    -- Data
...
function TradeListGUI:RegisterSlash()
    local options = {
        trade = {
            type = "execute",
            name = "Trade List",
            desc = ILM.L["Toggle Trade List window display"],
            handler = self,
            func = "Toggle",
        }
    }
...
ILM.GUI.TradeList = TradeListGUI