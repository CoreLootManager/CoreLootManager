...
local function ScanTooltip(self)
    ...
    for i = 1, self.fakeTooltip:NumLines() do
        local line = _G["ILMAutoAssignBagItemCheckerFakeTooltipTextLeft" .. i]
        if line then
...
function BagItemChecker:Initialize()
    self.fakeTooltip = CreateFrame("GameTooltip", "ILMAutoAssignBagItemCheckerFakeTooltip", UIParent, "GameTooltipTemplate")
...
function AutoAssign:Initialize()
    ...
    ILM.MODULES.EventManager:RegisterWoWEvent({"TRADE_SHOW"}, (function()
        Clear(self)
    ...
    ILM.MODULES.EventManager:RegisterWoWEvent({"TRADE_ACCEPT_UPDATE"}, (function()
        if not self.lastTradeTarget then return end
        HandleTradeAcceptUpdate(self)
    end))
    ILM.MODULES.EventManager:RegisterWoWEvent({"UI_INFO_MESSAGE"}, (function(_, _, _, message)
        if not self.lastTradeTarget then return end
    ...
    ILM.MODULES.EventManager:RegisterWoWEvent({"LOOT_OPENED"}, (function() lootWindowIsOpen = true end))
    ILM.MODULES.EventManager:RegisterWoWEvent({"LOOT_CLOSED"}, (function() lootWindowIsOpen = false end))
    ILM.MODULES.EventManager:RegisterEvent(ILM.CONSTANTS.EVENTS.GLOBAL_LOOT_REMOVED, function(_, data)
...
function AutoAssign:Track(itemLink, player)
    ...
    --
    ILM.GUI.TradeList:Refresh(true)
end
...
function AutoAssign:Handle(itemLink, target)
    if not self:IsIgnored(itemLink) then
        if ILM.MODULES.AuctionManager:GetAutoAssign() and lootWindowIsOpen then
            self:GiveMasterLooterItem(itemLink, target)
        elseif ILM.MODULES.AuctionManager:GetAutoTrade() then
...
ILM.MODULES.AutoAssign = AutoAssign
--@do-not-package@
