...
local scanTooltip = CreateFrame("GameTooltip", "ILMAuctionItemScanTooltip", UIParent, "GameTooltipTemplate")
local scanItem
...
function AuctionItem:GetTopBids(cutoff, type)
    ...
    type = ILM.CONSTANTS.BID_TYPES[type] and type or nil
...
function AuctionItem:HasValidBids()
    if self.hasValidBids == nil then
        ...
        for _, response in pairs(self.userResponses) do
            if not ILM.CONSTANTS.BID_TYPE_REMOVING_BIDS[response:Type()] then
...
ILM.MODELS.AuctionItem = AuctionItem