...
local AuctionItem = ILM.MODELS.AuctionItem
...
function AuctionInfo:UpdateRoster(roster)
    assertTypeof(roster, ILM.MODELS.Roster)
...
function AuctionInfo:UpdateRaid(raid)
    assertTypeof(raid, ILM.MODELS.Raid)
    if self:IsInProgress() then
...
function AuctionInfo:HandleRoll(who, roll)
    ...
    if not response then
        response = ILM.MODELS.UserResponse:New()
...
ILM.MODELS.AuctionInfo = AuctionInfo
