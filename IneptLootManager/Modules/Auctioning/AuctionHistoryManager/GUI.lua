...
local function InitializeDB(self)
    self.db = ILM.MODULES.Database:GUI('auctionHistory', {
    ...
    RightClickMenu = ILM.UTILS.GenerateDropDownMenu(
        {
            ...
            {
                title = ILM.L["Remove auction"],
                func = (function()
                    local rowData = self.st:GetRow(self.st:GetSelection())
                    if not rowData or not rowData.cols then return end
                    ILM.MODULES.AuctionHistoryManager:Remove(ST_GetItemSeq(rowData))
                end),
                color = "cc0000"
            },
            {
            ...
            {
                title = ILM.L["Remove old"],
                func = (function()
                    ILM.MODULES.AuctionHistoryManager:RemoveOld()
                end),
                color = "cc0000"
            },
            {
...
function AuctionHistoryGUI:Create()
    ...
    f:SetTitle(ILM.L["Auction History"])
    f:SetStatusText("")
    f:SetLayout("Table")
...
function AuctionHistoryGUI:Refresh(visible)
    ...
    local data = {}
    local stack = ILM.MODULES.AuctionHistoryManager:GetHistory()
    -- Data
    local rowId = 1
    for seq, auction in ipairs(stack) do
        local row = {
            cols = {
                ...
                { value = auction.id },
                { value = auction.bids },
                { value = date(ILM.L["%Y/%m/%d %a %H:%M:%S"], auction.time) },
                { value = seq },
                { value = auction.names },
                { value = auction.rolls }
            }
...
function AuctionHistoryGUI:RegisterSlash()
    local options = {
        auction_history = {
            ...
            desc = ILM.L["Toggle Auction History window display"],
            handler = self,
            func = "Toggle",
        }
    }
...
ILM.GUI.AuctionHistory = AuctionHistoryGUI