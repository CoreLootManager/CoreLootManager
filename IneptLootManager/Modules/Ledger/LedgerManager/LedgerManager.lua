...
local function registerReceiveCallback(callback)
    ...
    ILM.MODULES.Comms:Register(ILM.COMM_CHANNEL.LEDGER.SYNC, callback, CONSTANTS.ACL.LEVEL.PLEBS)
    ILM.MODULES.Comms:Register(ILM.COMM_CHANNEL.LEDGER.DATA, callback, CONSTANTS.ACL.LEVEL.ASSISTANT)
end
...
local function createLedger(self, database)
    local ledger = LedgerLib.createLedger(
        ...
        (function(data, distribution, target, callbackFn, callbackArg)
            return ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.LEDGER.SYNC, data, distribution, target, "NORMAL")
        end), -- send
        registerReceiveCallback, -- registerReceiveHandler
        (function(entry, sender)
            return ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.ASSISTANT, UTILS.Disambiguate(sender))
        end), -- authorizationHandler
        (function(data, distribution, target, progressCallback)
            return ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.LEDGER.DATA, data, distribution, target, "NORMAL")
...
function LedgerManager:Initialize()
    self.activeDatabase = ILM.MODULES.Database:Ledger()
...
function LedgerManager:Enable()
    ...
    if ILM.GlobalConfigs:GetDisableSync() then
...
function LedgerManager:Cutoff()
    ...
    ILM.MODULES.Comms:Suspend(ILM.COMM_CHANNEL.LEDGER.SYNC)
    ILM.MODULES.Comms:Suspend(ILM.COMM_CHANNEL.LEDGER.DATA)
    self.disabled = true
end
...
function LedgerManager:ExitSandbox(apply)
    ...
    if apply then
        ...
        ILM.MODULES.Database:UpdateLedger(self.activeDatabase)
...
function LedgerManager:Wipe()
    ...
    local db = ILM.MODULES.Database:Ledger()
    wipe(db)
    collectgarbage()
    self:Enable()
end
...
ILM.MODULES.LedgerManager = LedgerManager
