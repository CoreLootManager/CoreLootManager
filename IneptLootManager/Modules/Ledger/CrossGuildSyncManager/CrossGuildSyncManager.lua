...
local XGUILD = ILM.L["Cross-guild sync"]
local MAX_ENTRIES = 4
local XSYNC_TYPE = {
    SOURCE = 1,
    TARGET = 2,
...
local function UpdateSync(self)
    ILM.MODULES.LedgerManager:ClearP2PSources()
    ILM.MODULES.LedgerManager:ClearP2PTargets()
    ILM.MODULES.TrustInfoProvider:ClearExternalTrusted()
    for i=1,MAX_ENTRIES do
        local id = tostring(i)
        if TARGETS[self.types[id]] then
            ILM.MODULES.LedgerManager:AddP2PTarget(self.players[id])
        end
        if SOURCES[self.types[id]] then
            ILM.MODULES.LedgerManager:AddP2PSource(self.players[id])
            ILM.MODULES.TrustInfoProvider:AddExternalTrusted(self.players[id])
        end
    end
end
...
local function RegisterConfigurationOptions(self)
    local options = {
        xguild_desc = {
            name = ILM.L["Cross-guild synchronisation allows connecting two guild data through proxy mechanism called tunneling. Three types of connection are available: Send to, Accept from or Both. |cff00cc00Accepting data from another player will merge the two guilds databases.|r Use this functionality only if you are absolutely sure you know what you are doing."],
            type = "description",
            width = "full",
            fontSize = "medium",
            order = 1
    ...
    for i=1,MAX_ENTRIES do
        ...
        options["xguild_player_name" .. id] = {
            name = ILM.L["Player"],
            desc = ILM.L["Case-sensitive Player name in the format: Player-RealmName"],
            type = "input",
            width = "double",
        ...
        options["xguild_sync_type" .. id] = {
            name = ILM.L["Type"],
            desc = ILM.L["|cff00cc00Send to:|r If you are trusted in your guild, you will also send whisper syncs to the player.\n|cff00cc00Accept from:|r You will be accepting syncs from the player.\n|cff00cc00Both:|r Both of the above."],
            type = "select",
            style = "radio",
            width = 1,
    ...
    ILM.MODULES.ConfigManager:Register(XGUILD, options)
end
...
ILM.MODULES.ConfigManager:AddGroup(XGUILD, true)

ILM.MODULES.CrossGuildSyncManager = CrossGuildSyncManager