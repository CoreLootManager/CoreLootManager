...
local CommsPrefix = "ILM"

local Comms = ILM.CORE:NewModule("Comms", {}, "AceComm-3.0")
...
function Comms:Register(prefix, callback, securityCallbackOrLevel, allowSelfReceive)
    ...
    elseif type(securityCallbackOrLevel) == "number" then
        self.securityCallbacks[prefix] = (function(name, _) return ILM.MODULES.ACL:CheckLevel(securityCallbackOrLevel, name) end)
    else
...
function Comms:OnReceive(prefix, message, distribution, sender)
    ...
    if tmp._v ~= COMMS_VERSION then
        ...
        if (tonumber(tmp._v) or 0) > COMMS_VERSION then
            ILM.MODULES.Version:OutOfDate(true)
        end
        return
    end
