...
ILM.UTILS.FadeIn = (function (frame, timeToFade, startAlpha, endAlpha, info)
    if not frame then return end
    local fadeInfo = info or {}
    fadeInfo.mode = 'IN'
    fadeInfo.timeToFade = timeToFade
...
ILM.UTILS.FadeOut = (function (frame, timeToFade, startAlpha, endAlpha, info)
    if not frame then return end
    local fadeInfo = info or {}
    fadeInfo.mode = 'OUT'
    fadeInfo.timeToFade = timeToFade
...
ILM.UTILS.StopFlash = FLASH.Stop
-- db('Alpha/Stop', function(frame, alpha)
--     FLASH:Stop(frame, alpha)
-- end)

-- -- Function to start a frame flashing
ILM.UTILS.StartFlash = function(frame, fadeInTime, fadeOutTime, flashDuration, showWhenDone, flashInHoldTime, flashOutHoldTime, syncId)
    if frame then
        local index = 1
