...
CONSTANTS.FILTERS_GUI = {
    [CONSTANTS.FILTER.IN_RAID] = ILM.L["In Raid"],
    [CONSTANTS.FILTER.ONLINE] = ILM.L["Online"],
    [CONSTANTS.FILTER.STANDBY] = ILM.L["Standby"],
    [CONSTANTS.FILTER.IN_GUILD] = ILM.L["In Guild"],
    [CONSTANTS.FILTER.NOT_IN_GUILD] = ILM.L["External"],
    [CONSTANTS.FILTER.MAINS_ONLY] = ILM.L["Mains"],
    [CONSTANTS.FILTER.LOCKED_ONLY] = ILM.L["Locked"]
}
...
local function SelectClasses(self, isSelect)
    for i=1,UTILS.GetNumClasses() do
...
function Filters:GetAceOptions()
    ...
    if self.rank and self.inGuild then
        for index, info in pairs(ILM.MODULES.TrustInfoProvider:GetRanks()) do
    ...
    if self.search then
        options[self.prefix .. "search"] = {
            name = ILM.L["Search"],
            desc = ILM.L["Search for player names. Separate multiple with a comma ','. Minimum 3 characters. Overrides filtering."],
            type = "input",
            set = (function(i, v)
                self.searchString = v
...
function Filters:Filter(playerName, playerClass, searchFieldsList)
    ...
    if self.inRaid and self.filters[CONSTANTS.FILTER.IN_RAID] then
        if ILM.MODULES.RaidManager:IsInActiveRaid() then
            local profile = ILM.MODULES.ProfileManager:GetProfileByName(playerName)
            if profile then
                status = status and ILM.MODULES.RaidManager:GetRaid():IsPlayerInRaid(profile:GUID())
            end
        else
            status = false
        end
    elseif self.inStandby and self.filters[CONSTANTS.FILTER.STANDBY] then
        if ILM.MODULES.RaidManager:IsInProgressingRaid() then
            local profile = ILM.MODULES.ProfileManager:GetProfileByName(playerName)
            if profile then
                status = status and ILM.MODULES.RaidManager:GetRaid():IsPlayerOnStandby(profile:GUID())
            end
        elseif ILM.MODULES.RaidManager:IsInCreatedRaid() then
            local profile = ILM.MODULES.ProfileManager:GetProfileByName(playerName)
            if profile then
                status = status and ILM.MODULES.StandbyStagingManager:IsPlayerOnStandby(ILM.MODULES.RaidManager:GetRaid():UID(), profile:GUID())
            end
        else
            status = false
        end
    ...
    local guildies = ILM.MODULES.TrustInfoProvider:GetGuildies()
