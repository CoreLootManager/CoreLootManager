...
define.await({
    "Constants",
    "LibStub:EventDispatcher",
    "GlobalConfigs",
    "L"
}, function(CONSTANTS, eventDispatcher, GlobalConfigs, L)
...
local function BidAcceptedAlertFrame_SetUp(self, data)
    ...
    value = (value ~= nil) and tostring(value) or tostring(data.value)
    self.Amount:SetText(string.format(L["Bid %s accepted!"], value))
end
...
local function BidDeniedAlertFrame_SetUp(self, data)
    local value = tonumber(data.value)
    value = (value ~= nil) and tostring(value) or tostring(data.value)
    self.Amount:SetText(string.format(L["Bid %s denied!"], value))
end
...
eventDispatcher.addEventListener(USER_RECEIVED_POINTS, function(event, data)
    if not GlobalConfigs:GetAlerts() then return end
